...
$('#map_canvas').gmap({'center': '20.016789684314304,77.69557234062495'}).bind('init', function() { 
	...
	$.getJSON( '/mobile/pgr/registerGrievance!getMarkersforMap.action',{mobileNumber:'<s:property value="mobileNumber"/>',searchMonth:'<s:property value="searchMonth"/>',searchYear:'<s:property value="searchYear"/>'}, function(data) { 
 ...
 errornotification("notice","No Complaints found to show on map");
		...
		$.each( data.markers, function(i, marker) {
		  if(marker.latitude!=0){
			...
			var multiplecomp = marker.content.split('~');	
			
			if(multiplecomp.length==1){
				var cont = marker.content.split('^');
				var infoData = "" ;
				var row ="";
				var compId = "";
				for(i=1;i<cont.length;i++) {
					compId = cont[0];
					if(i==1)
					row = "<li> <a href=/mobile/pgr/registerGrievance!viewComplaintDetails.action?complaintId="+compId+'>'+cont[i]+ "</a></li>";
					else
					row = "<li>"+ cont[i]+ "</li>";
					infoData = infoData + row;
				}
				$('#map_canvas').gmap('openInfoWindow', { 'content':  infoData,maxwidth:200 }, this);
			}else{
			
				var multcont = marker.content.split('~');
				var infowindowdtls="";
			     for(i=1;i<multcont.length;i++) {
				infowindowdtls=infowindowdtls+prepareinfowindow(multcont[i]);
				}
			$('#map_canvas').gmap('openInfoWindow', { 'content':infowindowdtls,maxWidth: 200},this);
...
function prepareinfowindow(content){
...
<s:hidden id="searchYear" name="searchYear" value="%{searchYear}"/>
<s:hidden id="searchMonth" name="searchMonth" value="%{searchMonth}"/>
		<div data-theme="b" data-role="header" data-position="fixed">
		
		<a href="/mobile/pgr/registerGrievance!newform.action" data-icon="home" data-transition="none"
					data-ajax="false" data-iconpos="notext" data-theme="b" class="ui-btn-right">Home</a>
